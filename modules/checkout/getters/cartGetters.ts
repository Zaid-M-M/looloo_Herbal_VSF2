...
export const getItemPrice = (product: CartItemInterface): Price => {
  ...
  if (product.prices) {
    return {
      regular: product.prices.row_total_including_tax.value || 0,
...
export const getTotals = (cart: Cart): Totals => {
  ...
  return {
    ...
    total: (cart.prices.subtotal_including_tax.value) - calculateDiscounts(cart.prices.discounts) +  getShippingPrice(cart),
